/**
 * search_eu_implementations — Search EU directives/regulations with Austrian implementation info.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_to?: number;
  community?: string;
  has_austrian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS austrian_statute_count,
  ...
  if (input.has_austrian_implementation !== undefined) {
    sql += input.has_austrian_implementation ? ` HAVING austrian_statute_count > 0` : ` HAVING austrian_statute_count = 0`;
  }
  ...
  interface Row {
    ...
    austrian_statute_count: number; primary_implementations: string | null;
  }
  ...
  return {
    results: {
      results: rows.map(r => ({
        ...
        austrian_statute_count: r.austrian_statute_count,
